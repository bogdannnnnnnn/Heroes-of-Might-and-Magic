...
public class Test {
    ...
    // Тест 1. Завершение игры: победа игрока
    ...
    // Тест 2. Завершение игры: победа бота
    @org.junit.jupiter.api.Test
    ...
    public void testHeroMovementDeductsPoints() {
        ...
        class TestGamemap extends Gamemap {
            ...
            public char getCell(int x, int y) {
                // Для клетки (2,3) возвращаем препятствие
                if (x == 2 && y == 3) {
                    return '#';
                }
            ...
            public boolean canMoveToForHero(int x, int y, char heroIcon) {
                // Если координаты совпадают с (2,3) – считаем, что препятствие
                if (x == 2 && y == 3) {
                    return false;
                }
    ...
    public void testHeroCannotMoveToObstacle() {
        ...
        // Попытка переместится вправо (на (2,3), где препятствие)
    ...
    // Тест 10. Логика героя: принудительное перемещение
    @org.junit.jupiter.api.Test
    ...
    // Тест 12. Работа здания Tavern: применение эффекта
    @org.junit.jupiter.api.Test
    ...
    // Тест 13. Работа зданий: проверка отображения списка построек замка
    @org.junit.jupiter.api.Test
    ...
    // Тест 14. Действия нейтрального замка: метод siegeTurn – удаление охранного юнита
    @org.junit.jupiter.api.Test
    public void testNeutralCastleSiegeTurn() {
        ...
        assertTrue(neutralCastle.isCaptured(), "После достаточного числа вызовов siegeTurn замок должен считаться захваченным (");
    }

    // Тест 15. Действия нейтрального замка: захват замка
    @org.junit.jupiter.api.Test
    ...
    // Тест 16. Отображение игрового поля: обновление ячейки при перемещении героя
    @org.junit.jupiter.api.Test
    ...
    // Тест 17. Инициализация карты: проверка установки символов замков
    @org.junit.jupiter.api.Test
    ...
    // Тест 18. Отображение игрового поля: проверка размещения золотых куч
    @org.junit.jupiter.api.Test
    ...
    // Тест 19. Отображение игрового поля: размещение нейтрального замка
    @org.junit.jupiter.api.Test
    ...
    // Тест 20. Отображение игрового поля: корректный расчёт стоимости перемещения по различным типам клеток
    @org.junit.jupiter.api.Test
    ...
    // Тест 21. Отображение игрового поля: удаление золота с карты
    @org.junit.jupiter.api.Test
    ...
    // Тест 22. Отображение игрового поля: удаление сущности с карты
    @org.junit.jupiter.api.Test
    ...
    // Тест 23. Логика героя: сбор золота при перемещении на клетку с '$'
    @org.junit.jupiter.api.Test
    public void testHeroCollectsGold() {
        ...
        assertTrue(goldX != -1, "Должна быть размещена хотя бы одна куча золота");
        // Размещаем героя рядом с золотой кучей
        int heroX = goldX;
        int heroY = goldY - 1;
        if (heroY < 0) {
            heroY = goldY + 1;
    ...
    // Тест 24. Комплексный сценарий: последовательное выполнение нескольких действий
    @org.junit.jupiter.api.Test
    public void testComprehensiveScenario() {
        Hero hero = new Hero("Player", 2, 2, map);
        Castle castle = new Castle(2, 2);
        // Герой собирает золото
        map.placeGoldPiles(1);
        int goldX = -1;
        int goldY = -1;
        outer: for (int i = 0; i < map.getRows(); i++){
        ...
        // Добавляем в замок здание, необходимое для найма юнита
        castle.addBuilding("Собор");
        // Симулируем наём юнита: списываем стоимость найма и добавляем юнита
        int initialUnitCount = hero.getUnits().size();
        int hireCost = 30;
        hero.addGold(-hireCost);
        hero.addUnit("паладин");
